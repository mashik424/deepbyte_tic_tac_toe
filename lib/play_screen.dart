...
class PlayScreenState extends State<PlayScreen> {
  ...
  bool _isBotMoving =
      false; // Flag to check if the bot is currently making a move
  ...
  Widget _buildAnimatedText() {
    String turnMessage;
    if (widget.player2Mark == null) {
      turnMessage =
          _isUserTurn ? 'Your Turn (${widget.player1Mark})' : "Bot's Turn";
    } else {
      turnMessage = _isUserTurn ? "Player 1's Turn" : "Player 2's Turn";
    }
  ...
  void _handleCellTap(int row, int col) {
    if (!_isGameOver && _board[row][col] == null && _isUserTurn) {
      setState(() {
        _board[row][col] = widget.player1Mark;
        _isUserTurn = false;
        _checkGameStatus();
        if (!_isGameOver && widget.player2Mark == null) {
          _botMove();
        }
      });
    } else {
      setState(() {
        _board[row][col] = widget.player2Mark;
        _isUserTurn = true;
        _checkGameStatus();
      });
    }
    ...
    if (!_isGameOver && !_isUserTurn && !_isBotMoving) {
      ...
      Timer(const Duration(milliseconds: 800), () {
        ...
        if (emptyCells.isNotEmpty) {
          ...
          setState(() {
            _board[bestMove[0]][bestMove[1]] = getBotMark(widget.player1Mark);
            _isUserTurn = true;
            _checkGameStatus();
          });
        }
  ...
  int _minimaxWithAlphaBeta(List<List<String?>> board, int depth,
    ...
    if (isMaximizer) {
      ...
      for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
          if (board[i][j] == null) {
            board[i][j] = widget.player1Mark; // Use the user's mark
            int score =
  ...
  void _showGameOverDialog(String? winner) {
    ...
    if (winner == widget.player1Mark) {
      message = widget.player2Mark == null
    ...
    } else if (winner == getBotMark(widget.player1Mark)) {
      if (widget.player2Mark == null) {
        ...
        message = 'Congratulations! Player 2 Win!';
        iconData = Icons.sentiment_very_satisfied;
        iconColor = Colors.green;
        backgroundColor = Colors.green[50]!;
        buttonColor = Colors.green;
  ...
  void _restartGame() {
    setState(() {
      ...
      // If the bot plays first, call _botMove to make its first move
      if (!_isUserTurn && widget.player2Mark == null) {
        _botMove();
      }
    });
  }
